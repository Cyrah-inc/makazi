...
serve(async (req) => {
  ...
  try {
    ...
    // Require shared secret to be configured AND present in the callback URL
    const callbackSecret = Deno.env.get('MPESA_CALLBACK_SECRET');
    if (!callbackSecret) {
      console.error('mpesa-callback: MPESA_CALLBACK_SECRET not configured — rejecting request');
