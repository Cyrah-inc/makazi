...
const RentPage = () => {
  ...
  const sections = (
    <>
      ...
      <PropertyCarousel title="Luxury Rentals" subtitle="Premium high-end properties" properties={luxury.data ?? []} isLoading={luxury.isLoading} seeAllLink="/rent" icon={<Gem className="h-5 w-5 text-primary" />} />
      <PropertyCarousel title="Furnished Homes" subtitle="Move-in ready with all amenities" properties={furnished.data ?? []} isLoading={furnished.isLoading} seeAllLink="/rent" icon={<Sofa className="h-5 w-5 text-primary" />} />
      {county && (
        <PropertyCarousel title="Rentals Near You" subtitle={`Available in ${county} and surrounding areas`} properties={nearby.data ?? []} isLoading={nearby.isLoading} seeAllLink="/rent" icon={<Navigation className="h-5 w-5 text-primary" />} />
      )}
    </>
  );
